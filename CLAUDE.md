...
5. **OneSignal-Tags: es ist GENAU EINER** — `sched` trägt den kompletten Zeitplan: `v1;m=0500;e=1830;s=0530,0930,1000;t=ton-2` (Version; Morgen; Abend; SMALL-Zeiten; Ton nur bei Abweichung vom Standard; optional `o=v`, wenn das Web-Gerät noch auf der alten Domain loewenherz-app.vercel.app läuft — der Server lenkt den Push-Tap dann dorthin, sonst auf app.angstdoc.de). Zeiten immer UTC im Format `HHMM` auf dem 15-Minuten-Raster. Push aus = Tag gelöscht (leerer String). Gebaut in `buildTags()` (`js/push.js`) — dieselbe Funktion für Web und nativ; gewürfelt wird NUR in `rollIfNeeded()` (`js/small-schedule.js`).
